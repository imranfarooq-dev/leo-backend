alter table "public"."images" add column "next_image_id" integer;
...
CREATE OR REPLACE FUNCTION public.get_ordered_images_by_document_ids(document_ids integer[])
 RETURNS TABLE(id integer, document_id integer, image_name character varying, image_url character varying, image_path character varying, created_at timestamp without time zone, updated_at timestamp without time zone, next_image_id integer)
 LANGUAGE plpgsql
AS $function$
BEGIN
  RETURN QUERY
