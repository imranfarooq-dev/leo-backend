...
export class DocumentRepository {
  ...
  async createDocument(
    ...
    try {
      ...
      const document: Document = { ...data, images: [] };
      return document;
    } catch (error) {
  ...
  async fetchDocumentById(
    ...
    try {
      let data: DocumentDB | null = null;
      let error: Error | null = null;

      if (getFullImages) {
        ({ data, error } = await this.supabase.rpc('get_documents_by_ids', {
          p_document_ids: [documentId],
        }));
      } else {
        ({ data, error } = await this.supabase.rpc(
      ...
      let thumbnail_urls: string[] | undefined = undefined;

      if (getFullImages) {
        [image_urls, thumbnail_urls] = await Promise.all([
        ...
        thumbnail_urls = await this.supabaseService.getPresignedUrls(
          images.map((image) => image.filename),
          true,
        );
      }
      ...
      const processedImages = images
        ? await Promise.all(
            images.map(async (image, index) => {
              const { filename: _filename, ...imageRest } = image;
      ...
      this.logger.error(
        'Failed to fetch item by id with error: ' + error.message,
      );
    }
  }
    ...
    try {
      const [countResult, documentsResult] = await Promise.all([
        this.supabase
          ...
          .eq('user_id', userId),
        this.supabase.rpc('get_documents_by_user_id', {
          p_user_id: userId,
      ...
      if (countResult.error || documentsResult.error) {
        throw new Error(
          countResult.error?.message ??
            ...
            'Failed to fetch items by user id',
        );
      }
      ...
      if (documentsResult.data.length > 0) {
        ...
        const filenames = documentsResult.data.flatMap(
      ...
      const documentsWithThumbnailUrls: Document[] = await Promise.all(
      ...
      return {
        documents: documentsWithThumbnailUrls,
        count: countResult.count,
      };
    } catch (error) {
      this.logger.error(
        'Failed to fetch items by user_id for user ' +
          userId +
          ' with error: ' +
          error.message,
      );
  ...
  async fetchDocumentsByIdsWithImages(
    ...
    try {
      const { data, error } = await this.supabase.rpc('get_documents_by_ids', {
      ...
      this.logger.error(
        'Failed to fetch items by id with error: ' + error.message,
      );
    }
  }
  ...
  async userIdsFromDocumentIds(documentIds: string[]): Promise<string[]> {
    try {
      ...
      this.logger.error(
        'Failed to fetch user id from document id with error: ' + error.message,
      );
      throw error;
    }
  }
